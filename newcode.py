...
@bot.command()
async def listacodes(ctx):
    lista = [
        "$hello.-Te da la bienvenida el bot",
        "$heh.-Repite por cinco veces el texto",
        "$admin.- El bot escribe por ti",
        "$repetir.-Repite el mensaje escrito por el usuario",
        "$reglas.-Muestra la lista de reglas del server",
        "$codes.-Muestra la lista actual de codigos existentes en el server",
        "Puedes utilizar estos codes de acuerdo a tus necesidades",
    ]
    
    mensaje = "Codigos en funcionamiento:\n\n" + "\n".join(lista)
    await ctx.send(mensaje)
